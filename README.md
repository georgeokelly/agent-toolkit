...
│   └── README.md
│
├── skills/                      ← Agent skills / Agent 技能（默认部署到用户全局 skill 目录）
...
│       ├── gen-cursor.sh        # Cursor .mdc + global skills + worktrees.json / Cursor 产物生成
│       ├── gen-claude.sh        # CC .claude/rules + global skills + subagents / CC 产物生成
...
**Global skill targets / 全局 skill 目标目录：** `agent-sync` deploys skills to user-global paths by default, not into each workspace. Workspace-scoped skill directories from older agent-sync versions are removed only when their legacy manifest is present, so user-authored workspace skills remain untouched. / `agent-sync` 默认把 skills 部署到用户全局路径，不再写入每个 workspace。旧版本写入 workspace 的 skills 只有在存在 legacy manifest 时才会被回收，因此用户手写的 workspace skills 不会被删除。

| Tool / 工具 | Default global path / 默认全局路径 |
|---|---|
| Cursor | `~/.cursor/skills` (`~/.cursor/skills-cursor` is Cursor-managed built-in content; do not write user skills there) |
| Claude Code | `~/.claude/skills` |
| Codex | `~/.agents/skills` plus `${CODEX_HOME:-~/.codex}/skills` compatibility mirror |
...
agent-sync cc-skills .            # Only sync skills to ~/.claude/skills/ / 仅同步 skills 到用户全局 Claude 目录
agent-sync skills .               # Only sync skills to Cursor's user-global skills dir / 仅同步 skills 到用户全局 Cursor 目录
...
agent-sync opencode-skills .      # Only sync skills to ~/.config/opencode/skills/ / 仅同步 skills 到用户全局 OpenCode 目录
...
agent-sync clean .                # Remove project-scoped generated files; keep global skills / 清理项目级生成文件；保留全局 skills
```
...
| Skills | `~/.agents/skills/<name>/SKILL.md` + `${CODEX_HOME:-~/.codex}/skills/<name>/SKILL.md` | Official user-scope Codex skills plus CODEX_HOME compatibility mirror / Codex 官方用户级 skills 与 CODEX_HOME 兼容镜像 |
...
- `legacy` — Root `AGENTS.override.md` only (no `.codex/`, no `.agents/agents/`) / 仅生成 `AGENTS.override.md`
- `native` (default) — `.codex/config.toml` + user-global Codex skills + root `AGENTS.override.md` / 原生模式（完整产出）
...
| Skills | `~/.claude/skills/<name>/SKILL.md` | User-global Claude Code skills / 用户全局 Claude Code skills |
...
- `native` (default) — `.claude/rules/` + user-global `~/.claude/skills/` / CC 原生 rules + 用户全局 skills
...
- `~/.claude/skills/<name>/SKILL.md`: user-global skills, discovered natively / 用户全局技能，原生发现
...
`agent-sync` generates an OpenCode-native configuration at the project root plus user-global skills and project-scoped subagents:

`agent-sync` 在项目根目录生成 OpenCode 原生配置，同时生成用户全局 skills 与项目级 subagents：
...
| Skills | `~/.config/opencode/skills/<prefix><name>/SKILL.md` | User-global OpenCode skills, same prefix rules (HIST-005) / 用户全局 OpenCode skills，共用 HIST-005 前缀规则 |
...
- `off` — No project-scoped OpenCode output; agent-sync-managed `opencode.json` (stamp-gated) and `.opencode/` are reconciled away / 不生成项目级 OpenCode 输出；由 agent-sync 管理的 `opencode.json`（stamp-gated）和 `.opencode/` 会被回收
...
~/.codex/skills/gla-pre-commit/SKILL.md            (name: gla-pre-commit)
~/.config/opencode/skills/gla-pre-commit/SKILL.md  (name: gla-pre-commit)
```
...
**Scope / 作用范围：** core `skills/` **and** every `extras/<bundle>/skills/`. The prefix is idempotent — re-syncing never produces `gla-gla-…`; stale directories from the previous prefix are pruned via the user-global manifest. / 作用于核心 `skills/` 和所有 `extras/<bundle>/skills/`。幂等：反复 sync 不会产生 `gla-gla-…`；切换前缀时旧目录通过用户全局 manifest 自动清理。
...
| Skills deployment | User-global Cursor skills path matches manifest from rules repo / 用户全局 Cursor skills 已部署且与 manifest 一致 |
...
| CC skills deployment | `~/.claude/skills/` matches manifest (when CC Mode ≠ off) / 用户全局 CC skills 已部署且与 manifest 一致 |
| CC/Cursor consistency | `.claude/rules/` and `.cursor/rules/` have matching counts (when CC Mode ≠ off) / CC 与 Cursor rules 数量一致 |
| Codex config | `.codex/config.toml` is valid TOML with `child_agents_md` (when Codex Mode = native) / Codex 配置有效 |
| Codex skills | `~/.agents/skills/` plus `~/.codex/skills/` compatibility mirror match manifests (when Codex Mode = native) / Codex 官方用户级 skills 与 CODEX_HOME 兼容镜像已部署 |
...
| OpenCode skills | `~/.config/opencode/skills/` matches manifest (when OpenCode Mode = native) / 用户全局 OpenCode skills 已部署 |
...
| Want to remove project-scoped generated files / 需要清除项目级生成文件 | `agent-sync clean .` |
...
- **Auto-handled on the next `agent-sync` run** (zero action required) / **下一次 `agent-sync` 自动处理**（无需手动操作）
  - New `gla-<name>/` directories are deployed to user-global skill paths with frontmatter `name:` rewritten. Legacy workspace skill dirs (`.cursor/skills/`, `.claude/skills/`, `.agents/skills/`, `.opencode/skills/`) are removed only when an old agent-sync manifest is present. Verified by regression tests `T19a`/`T19b`/`T19e`. / 新 `gla-<name>/` 目录会部署到用户全局 skill 路径并重写 frontmatter。旧 workspace skill 目录仅在存在旧 agent-sync manifest 时回收。由 `T19a`/`T19b`/`T19e` 覆盖。
...
- **Scope** / **影响面**:
  ...
  - User-authored skills outside `skills/` and `extras/<bundle>/skills/` (e.g. a hand-curated workspace `.cursor/skills/mine/` or global custom skill) are **not** touched — `agent-sync` only manages deployment targets listed in its manifests. / 不受 agent-sync 管理的 skill 目录不会被改。
...
- **Auto-handled on the next `agent-sync` run** (zero action required) / **下一次 `agent-sync` 自动处理**（无需手动操作）
  ...
  - `~/.config/opencode/skills/<prefix><name>/` — user-global OpenCode skills with the same prefix rules; first sync creates it. / 首次 sync 时创建用户全局 OpenCode skills，前缀规则一致。
...
- **Opting out** / **关闭 OpenCode**:
  - Set `**OpenCode Mode**: off` in `.agent-local.md` and re-sync. `reconcile_mode_outputs()` removes stamp-gated project `opencode.json`, sweeps project-scoped OpenCode subagent manifests, and `rmdir`s `.opencode/` when empty. User-owned `opencode.json` (no stamp) is preserved; user-global skills are not removed by a project mode toggle. Covered by `T21` / `T22`. / 在 overlay 加 `**OpenCode Mode**: off` 后重新 sync。`reconcile_mode_outputs()` 会移除带 stamp 的项目级 `opencode.json`、回收项目级 OpenCode subagent manifest，并在 `.opencode/` 空时 `rmdir`。用户手写的 `opencode.json` 不受影响；用户全局 skills 不会因为项目 mode 切换被删除。
